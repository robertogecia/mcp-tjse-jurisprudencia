...
def partes_fts(consulta: str | None, grupos: list[list[str]] | None, exato: bool = False) -> list[tuple[str, str, bool]]:
    ...
    for g in grupos or []:
        ...
        partes.append(("[" + " | ".join(g) + "]", "(" + " OR ".join(fr) + ")", True))
    if consulta:
...
def montar_fts(consulta: str | None, grupos: list[list[str]] | None, exato: bool = False) -> str:
    ...
    return expressao_fts(partes_fts(consulta, grupos, exato))
...
def _buscar(consulta, grupos, orgao, classe, relator, numero, por_pagina, pagina, data_inicio, data_fim,
    ...
    if por_bm25:
        if usa_cobertura:
            ...
            base = (f"(SELECT acordao ac, bm25({tab_fts}, {pesos}) rk FROM {tab_fts} WHERE {tab_fts} MATCH ?) r "
                    ...
                    f"LEFT JOIN (SELECT acordao, COUNT(*) k FROM ({uni_sql}) GROUP BY acordao) cob ON cob.acordao = a.acordao")
            args = [q_fts] + uni_args + args
        else:
            base = (f"(SELECT acordao ac, bm25({tab_fts}, {pesos}) rk FROM {tab_fts} WHERE {tab_fts} MATCH ?) r "
                    "JOIN acordaos a ON a.acordao = r.ac JOIN edicoes e USING(edicao)")
            args = [q_fts] + args
    ...
    if por_bm25:
        ordem_rk = "COALESCE(cob.k, 0) DESC, r.rk" if usa_cobertura else "r.rk"
        rows = con.execute(f"SELECT a.*, e.data ed_data{', COALESCE(cob.k, 0) cobertura' if usa_cobertura else ''}{sql} "
                           f"ORDER BY {ordem_rk}, a.edicao DESC, a.acordao DESC LIMIT ? OFFSET ?",
    ...
    for r in rows:
        ...
        cob_txt = f" · casa {r['cobertura']}/{n_uni} termos" if usa_cobertura else ""
        out.append(f"■ Acórdão {r['acordao']} · processo {r['processo']} · {r['recurso'] or r['classe']}{cob_txt}\n"
